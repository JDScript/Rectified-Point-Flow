...
class FlowVisualizationCallback(VisualizationCallback):
    ...
    def on_test_batch_end(
        ...
        pointclouds_pred_list = [
            traj[-1].reshape(B, -1, 3) for traj in trajectories_list
        ]
        ...
        if self.scale_to_original_size:
            ...
            pointclouds_pred_list = [
                pred * scale[:, None, None] for pred in pointclouds_pred_list
            ]
        ...
        for i in range(B):
            ...
            for n in range(K):
                ...
                if self.save_trajectory:
                    ...
                    num_steps = trajectory.shape[0]
                    trajectory = trajectory.reshape(num_steps, B, -1, 3).permute(
                        1, 0, 2, 3
                    )  # (bs, num_steps, N, 3)
                    if self.scale_to_original_size:
                        trajectory = (
                            trajectory * scale[:, None, None, None]
                        )  # (bs, num_steps, N, 3)
            ...
            if (
                ...
                and i >= self.max_samples_per_batch
            ):
                break
...
class OverlapVisualizationCallback(VisualizationCallback):
    ...
    def setup(self, trainer, pl_module, stage):
        ...
        rr.init("overlap_visualization", spawn=False)
        rr.serve_grpc(grpc_port=1116)
        print("Rerun server started at port 1116")
    ...
    def on_test_batch_end(
        ...
        for i in range(B):
            ...
            cumsum = torch.cumsum(batch["points_per_part"][i], dim=0)
            cumsum = torch.nn.functional.pad(cumsum, (1, 0), value=0)
            for j in range(len(cumsum) - 1):
                start, end = cumsum[j], cumsum[j + 1]
                rr.log(
                    [sample_name, f"part_{j}"],
                    rr.Points3D(
                        positions=pts_gt[i][start:end].cpu().numpy(),
                        colors=colors[start:end].cpu().numpy(),
                    ),
                )
